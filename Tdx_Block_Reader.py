...
# 通达信本地板块文件路径，需根据实际情况修改
block_file_path = 'C:\\Users\\flash\\Documents\\myData\stock\\tdx_block\\local\\block_gn.dat'
...
def read_tdx_blocks(file_path):
    # 创建 BlockReader 对象并读取板块文件
    code = ""
    blocks = BlockReader().get_df(file_path)

    if not blocks.empty:
        #清除上次执行的数据
        clear_specific_files(OUTPUT_DIR_NOW, ".txt")
        print("成功读取通达信本地板块信息：")
        # 输出板块信息
...
if __name__ == "__main__":
    read_tdx_blocks(block_file_path)
